...
def parse_predictions_SDCoT_dynamic(
    predicted_boxes, sem_cls_logits, sem_cls_probs, objectness_probs, point_cloud, config_dict, use_cls_threshold=False, threshold_list=None, return_all_cls_probs=False
):
    """
    This differs from parse_predictions_SDCoT in that it uses logits instead of probabilities.
    Because balanced softmax will be used to compute the probabilities, the logits are needed.
    ...
    sem_cls_probs = sem_cls_probs.detach().cpu().numpy()  # B,num_proposal,10
    sem_cls_logits = sem_cls_logits.detach().cpu().numpy()  # B,num_proposal,10
