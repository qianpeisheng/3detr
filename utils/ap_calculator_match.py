...
def parse_predictions_SDCoT(
    predicted_boxes, sem_cls_probs, objectness_probs, point_cloud, config_dict, threshold_list
):
    """Parse predictions to OBB parameters and suppress overlapping boxes
    ...
    # sem_cls_probs is of shape (1, num_proposal, num_base_class)
    # the threshold_list is a dictionary of thresholds for each class
    # sum the number of proposals that are above the threshold for each class
    counts = np.sum(sem_cls_probs > threshold_list, axis=1)

    # normalizing counts between 0 and 1
    counts = counts / counts.max()

    # # update threshold_list by multiplying by the counts if the counts are greater than 0
    # for i in range(len(threshold_list)):
    #     if counts[0, i] > 0:
    #         threshold_list[i] = threshold_list[i] * counts[0, i]
        ...
        if config_dict["per_class_proposal"]:
            ...
            for ii in range(config_dict["dataset_config"].num_base_class):
                cur_list += [
                    ...
                    for j in range(pred_corners_3d_upright_camera.shape[1])
                    if pred_mask[i, j] == 1
                    and obj_prob[i, j] > config_dict["pseudo_objectness_conf_thresh"]
                    and sem_cls_probs[i, j, ii] > config_dict["pseudo_class_conf_thresh"]
                ]
            batch_pred_map_cls.append(cur_list)
        elif config_dict["use_cls_confidence_only"]:
            batch_pred_map_cls.append(
...
def get_ap_config_dict(
    ...
    conf_thresh=0.05,
    pseudo_objectness_conf_thresh=0.95,
    pseudo_class_conf_thresh=0.9,
    no_nms=False,
    dataset_config=None,
):
    """
    ...
    config_dict = {
        ...
        'pseudo_objectness_conf_thresh': pseudo_objectness_conf_thresh,
        'pseudo_class_conf_thresh': pseudo_class_conf_thresh,
    }
    return config_dict
...
class APCalculator(object):
    ...
    def step_SDCoT(
        ...
        # same as step but return batch_pred_map_cls and stop there
        batch_pred_map_cls = parse_predictions(
            predicted_box_corners,
            sem_cls_probs,
            objectness_probs,
